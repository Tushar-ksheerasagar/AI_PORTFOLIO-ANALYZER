...
const elements = {
  ...
  // Smart Rebalancer Panel
  runRebalancerBtn: document.getElementById("run-rebalancer-btn"),
  rebalancerToggleFloating: null,
  rebalancerFloatingWidget: null,
  rebalancerCloseFloating: null,
...
function showAuthView() {
  ...
  if (elements.chatToggleFloating)
    elements.chatToggleFloating.classList.add("hidden");
...
function setupEventListeners() {
  ...
  elements.authToggleLink.addEventListener("click", (e) => {
    ...
    if (isLogin) {
      ...
      elements.authSubtitle.textContent =
        "Sign up to begin building your investment portfolio.";
      ...
      elements.authSubtitle.textContent =
        "Analyze your investments with advanced metrics and AI commentary.";
  ...
  elements.deletePortfolioBtn.addEventListener("click", async () => {
    ...
    const pName = state.portfolios.find(
      (p) => p.id === state.selectedPortfolioId,
    )?.name;
    if (
      !confirm(
        `Are you sure you want to delete the portfolio '${pName}' and all its holdings?`,
      )
    ) {
      return;
    }
    try {
      await apiRequest(`/api/portfolios/${state.selectedPortfolioId}`, {
        method: "DELETE",
      });
      ...
      elements.currentPortfolioName.textContent =
  ...
  elements.addHoldingForm.addEventListener("submit", async (e) => {
    ...
    try {
      showToast(
        `Adding position '${ticker}'... Fetching live data...`,
        "success",
      );
      await apiRequest(
        `/api/portfolios/${state.selectedPortfolioId}/holdings`,
        {
          method: "POST",
  ...
  if (elements.sidebarLogoLink) {
    elements.sidebarLogoLink.addEventListener("click", async (e) => {
      ...
      if (elements.rebalancerToggleFloating)
        elements.rebalancerToggleFloating.classList.add("hidden");
  ...
  if (elements.chatForm) {
    elements.chatForm.addEventListener("submit", async (e) => {
      ...
      const loadingBubble = appendChatMessage(
        "loading",
        '<i class="fa-solid fa-spinner fa-spin"></i> Thinking...',
      );

      try {
        const chatData = await apiRequest(
          `/api/portfolios/${state.selectedPortfolioId}/insights/chat`,
          {
            method: "POST",
            body: {
              message: message,
  ...
  if (elements.runRebalancerBtn) {
    elements.runRebalancerBtn.addEventListener("click", async () => {
      ...
      if (elements.rebalancerModal)
        elements.rebalancerModal.classList.remove("hidden");
      if (elements.rebalancerModalLoading)
        elements.rebalancerModalLoading.classList.remove("hidden");
      if (elements.rebalancerModalOutput)
        elements.rebalancerModalOutput.innerHTML = "";

      try {
        const data = await apiRequest(
          `/api/portfolios/${state.selectedPortfolioId}/insights/rebalance`,
          {
            method: "POST",
          },
        );
        ...
        if (elements.rebalancerModalLoading)
          elements.rebalancerModalLoading.classList.add("hidden");
        ...
        html += `</div>`;
        if (elements.rebalancerModalOutput)
          elements.rebalancerModalOutput.innerHTML = html;
      } catch (err) {
        if (elements.rebalancerModalLoading)
          elements.rebalancerModalLoading.classList.add("hidden");
        if (elements.rebalancerModalOutput) {
  ...
  const closeRebalancerModal = () => {
    if (elements.rebalancerModal)
      elements.rebalancerModal.classList.add("hidden");
  };

  if (elements.closeRebalancerModalBtn) {
    elements.closeRebalancerModalBtn.addEventListener(
      "click",
      closeRebalancerModal,
    );
  }
  if (elements.closeRebalancerModalFooterBtn) {
    elements.closeRebalancerModalFooterBtn.addEventListener(
      "click",
      closeRebalancerModal,
    );
  }
...
async function selectPortfolio(id) {
  ...
  if (portfolio) {
    ...
    if (elements.chatToggleFloating)
      elements.chatToggleFloating.classList.remove("hidden");
...
async function refreshDashboard(isBackground = false) {
  ...
  try {
    ...
    const analytics = await apiRequest(
      `/api/portfolios/${state.selectedPortfolioId}/analytics`,
    );
...
function renderHoldingsTable() {
  ...
  state.analytics.holdings.forEach((h) => {
    ...
    const changeClass =
      h.absolute_return >= 0 ? "text-positive" : "text-negative";
...
async function deletePosition(ticker) {
  if (
    !confirm(
      `Are you sure you want to remove all shares of '${ticker}' from this portfolio?`,
    )
  ) {
    return;
  }
  try {
    ...
    const portfolioDetails = await apiRequest(
      `/api/portfolios/${state.selectedPortfolioId}`,
    );
    const holding = portfolioDetails.holdings.find((h) => h.ticker === ticker);
    if (holding) {
      await apiRequest(
        `/api/portfolios/${state.selectedPortfolioId}/holdings/${holding.id}`,
        {
          method: "DELETE",
        },
      );
...
async function fetchAIInsights(force = false) {
  ...
  try {
    ...
    elements.insightSummary.textContent =
      "Could not load AI commentary. Please ensure you have added holdings to compute analytics.";
  } finally {
...
function resetAIInsightsUI() {
  state.insights = null;
  elements.insightSummary.textContent =
    "AI Analysis is currently pending. Click the 'Run AI Diagnostics' button above to analyze your portfolio metrics and generate insights.";
  elements.insightRisk.textContent =
    "Click 'Run AI Diagnostics' above to generate risk analysis.";
  elements.insightDiversification.textContent =
    "Click 'Run AI Diagnostics' above to generate diversification commentary.";
  elements.insightWatchList.innerHTML =
    "<li>Analysis pending. Click the button above to run diagnostics.</li>";
  elements.insightDisclaimer.textContent =
    "Disclaimer: Educational commentary only.";
}
...
function formatChatText(text) {
  ...
  if (
    text.trim().startsWith("<i class=") ||
    text.trim().startsWith("<span") ||
...
function updateChartThemeColors() {
  ...
  if (charts.allocation) {
    if (charts.allocation.options.plugins.legend) {
      charts.allocation.options.plugins.legend.labels.color =
        colors.legendColor;
    }
...
function renderTreemap() {
  ...
  if (!holdings || holdings.length === 0) {
    ...
    ctx.fillText(
      "Add holdings to see your asset treemap",
...
function squarify(items, rect, totalValue) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (
      row.length === 0 ||
      worstAspectRatio(testRow, testArea, side) <=
        worstAspectRatio(row, rowArea, side)
    ) {
...
function initWealthSimulator() {
  ...
  function updateCalculation() {
    ...
    if (monthlyVal)
      monthlyVal.textContent = `₹${Math.round(P).toLocaleString("en-IN")}`;
    if (yearsVal)
      yearsVal.textContent = `${years} ${years === 1 ? "Year" : "Years"}`;
    if (rateVal) rateVal.textContent = `${annualRate.toFixed(1)}%`;

    if (totalWealthEl)
      totalWealthEl.textContent = `₹${Math.round(fv).toLocaleString("en-IN")}`;
    if (totalInvestedEl)
      totalInvestedEl.textContent = `₹${Math.round(totalInvested).toLocaleString("en-IN")}`;
    if (totalGainEl)
      totalGainEl.textContent = `₹${Math.round(totalGain).toLocaleString("en-IN")}`;
    if (wealthGrowthEl)
      wealthGrowthEl.textContent = `+${multiplier.toFixed(1)}% Wealth Multiplier`;
  }
